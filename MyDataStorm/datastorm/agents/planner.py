...
class PlannerAgent:
    ...
    def generate_tree_based_questions(
        ...
        Args:
            topic: 用户查询/主题
            question_nodes: 完整的问题树节点列表 (包含所有历史问题及答案)
            insights: 当前全局洞察库
            thesis: 当前论点 (可能为 None)

        Returns:
            (follow_up_questions, exploratory_questions) 两个列表
        """
        ...
        # 下限检查: 如果 LLM 返回不足 m/n, 记录警告但不强制补齐
        if len(follow_ups) < m:
            logger.warning(
        ...
        logger.info(
            "Tree-based: %d follow-up + %d exploratory questions generated (lower bounds: m=%d, n=%d)",
